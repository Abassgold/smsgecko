...
/** Paginated, filterable orders table — the full /admin/orders listing, and
 * (passing `userId`) the same table scoped to one user's orders, with the
 * now-redundant User column dropped. */
export function OrdersTable({ userId }: { userId?: string }) {
  const [status, setStatus] = useState('all');
  const [q, setQ] = useState('');
  const [page, setPage] = useState(1);
  const orders = useAdminOrders({ status, q, userId, page });
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="overflow-x-auto">
        {orders.isLoading ? (
          ...
          <table className={`w-full text-sm ${userId ? 'min-w-[1020px]' : 'min-w-[1220px]'}`}>
            <thead>
